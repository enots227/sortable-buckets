...
export type InputInstance<TValue> = {
    ...
    getItem: (index: BucketItemIndexPair) => BucketItem<TValue>
    /**
     * Get the bucket item from a value
     * @param itemValue The value of the bucket item
     * @returns The bucket item
     */
    getItemFromValue: (itemValue: TValue) => BucketItem<TValue> | undefined 
    getBuckets: () => Bucket[]
    /**
     ...
     */
    moveItemToBucket: (index: BucketItemIndexPair, bucketAdjustment: number, itemValue?: TValue) => void
}
...
export type InitialInputState<TValue> = PartialKeys<InputState<TValue>, 'dragging' | 'globalFilter'>;
...
export type ResolvedOptions<TValue> = RequiredKeys<Options<TValue>, 
    'remainingBucket'
    | 'debugAll'
    | 'debugInput'
